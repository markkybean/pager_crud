<?php
    ...
    // var_dump($_POST['pager_event_action'],$_POST['txtfld']);
            ...
            foreach ($xarr_advs as $key => $value) {
                    if (count($value) > 1) {
                            if ($value['from'] != "") {
                                    if ($xex_type[array_search($key, $xex_fields)] == 'date' 
...
<script>
    function load_dialog(xpar) {
        ...
        switch(xpar) {
            case "ADD": 
                xtitle = "Add Employee";
                xbuttons = {
                    "Save" : function (){ pager_save_employee(xpar); },
                    "Close" : function () { jQuery(this).dialog('close'); }
                }
                break;
                
            case "VIEW": 
                xtitle = "View Employee";
                xbuttons = {
                    "Close" : function () { jQuery(this).dialog('close'); }
                }
                break;
    ...
    function pager_save_employee(xpar){
        if (xpar === "ADD"){
            var xdata = $('#form_data').serialize()+"&event_action=save_transaction";
            $.ajax({
                url: "ajax_crud.php",
                type: "POST",
                dataType: "json",
    ...
    function pager_add_click() {
        ...
        $.ajax({
            ...
            success: function(xres){
                $('#div_info').empty('');
                $('#div_info').append(xres.form);
                $('#div_info').dialog('open');
            }
        });
    }

    function pager_view_click(xrecid){
        load_dialog("VIEW");
    ...
    function pager_edit_click(xrecid){
        load_dialog("EDIT");
    ...
    function pager_delete_click(xrecid){
    ...
    function pager_export_click(){
        document.forms.myform.method = 'POST';
        document.forms.myform.target = '_blank';
        document.forms.myform.action = 'pdf_employeelist.php';
        document.forms.myform.txt_repoutput.value = "tab";
        document.forms.myform.submit();
    }
</script>
<?php
